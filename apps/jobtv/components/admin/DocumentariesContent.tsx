...
import {
  getAdminTopPageDocumentaries,
  createTopPageDocumentary,
  updateTopPageDocumentary,
  deleteTopPageDocumentary,
  reorderTopPageDocumentaries
} from "@/lib/actions/top-page-documentary-actions";
...
function SortableDocumentaryItem({
  item,
  onEdit,
  onDelete,
  isSortable
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="truncate text-sm text-gray-500">{item.channel}</p>
        {item.link_url && (
          <p className="truncate text-xs text-gray-400">{item.link_url}</p>
        )}
      </div>
      <button
        type="button"
...
function DocumentaryForm({
  ...
  initialValues?: { title: string; channel: string; link_url: string };
    ...
    <form ref={formRef} onSubmit={handleSubmit} className="space-y-3">
      ...
      <div>
        ...
        <input
          type="text"
          name="channel"
          defaultValue={initialValues?.channel ?? "JOBTV"}
          className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-black focus:outline-none focus:ring-1 focus:ring-black"
          placeholder="JOBTV"
        />
      </div>
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">
          サムネイル画像{!initialValues && <span className="text-red-500"> *</span>}
...
export default function DocumentariesContent() {
  ...
  const fetchItems = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await getAdminTopPageDocumentaries();
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    setActionError(null);
    const res = await reorderTopPageDocumentaries(newOrder.map((i) => i.id));
  ...
  const handleCreate = async (formData: FormData) => {
    setActionError(null);
    const res = await createTopPageDocumentary(formData);
    if (res.error) {
      setActionError(res.error);
      return;
    }
  ...
  const handleUpdate = async (formData: FormData) => {
    ...
    setActionError(null);
    const res = await updateTopPageDocumentary(editingItem.id, formData);
    if (res.error) {
      setActionError(res.error);
      return;
    }
  ...
  const handleDelete = async (id: string) => {
    setActionError(null);
    const res = await deleteTopPageDocumentary(id);
    if (res.error) {
      setActionError(res.error);
      return;
    }
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-8 lg:grid-cols-2">
        <section className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          <h2 className="mb-4 text-lg font-semibold text-gray-900">現在のドキュメンタリー一覧</h2>
          {items.length === 0 ? (
            <EmptyState
              title="ドキュメンタリーがありません"
              description="右のフォームからドキュメンタリーを追加してください"
            />
          ) : (
            <DndContext
              sensors={sensors}
                    ...
                    <React.Fragment key={item.id}>
                      ...
                      {editingItem?.id === item.id && (
                        <li className="rounded-lg border border-blue-200 bg-blue-50 p-4">
                          <p className="mb-3 text-sm font-medium text-blue-800">ドキュメンタリーを編集</p>
                          <DocumentaryForm
                            initialValues={{
        ...
        <section className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          <h2 className="mb-4 text-lg font-semibold text-gray-900">新規ドキュメンタリーを追加</h2>
          <DocumentaryForm onSubmit={handleCreate} submitLabel="追加する" />
        </section>
      </div>
    </div>
  );
