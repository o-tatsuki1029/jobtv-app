...
import {
  getAdminTopPageShunDiaries,
  createTopPageShunDiary,
  updateTopPageShunDiary,
  deleteTopPageShunDiary,
  reorderTopPageShunDiaries
} from "@/lib/actions/top-page-shundiary-actions";
...
function SortableShunDiaryItem({
  item,
  onEdit,
  onDelete,
  isSortable
...
export default function ShunDiariesContent() {
  const [items, setItems] = useState<ShunDiary[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [actionError, setActionError] = useState<string | null>(null);
  const [isReordering, setIsReordering] = useState(false);
  const [editingItem, setEditingItem] = useState<ShunDiary | null>(null);

  const fetchItems = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
      const res = await getAdminTopPageShunDiaries();
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    setActionError(null);
    const res = await reorderTopPageShunDiaries(newOrder.map((i) => i.id));
  ...
  const handleCreate = async (formData: FormData) => {
    setActionError(null);
    const res = await createTopPageShunDiary(formData);
    if (res.error) {
      setActionError(res.error);
      return;
    }
  ...
  const handleUpdate = async (formData: FormData) => {
    ...
    setActionError(null);
    const res = await updateTopPageShunDiary(editingItem.id, formData);
    if (res.error) {
      setActionError(res.error);
      return;
    }
  ...
  const handleDelete = async (id: string) => {
    setActionError(null);
    const res = await deleteTopPageShunDiary(id);
    if (res.error) {
      setActionError(res.error);
      return;
    }
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-8 lg:grid-cols-2">
        <section className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          <h2 className="mb-4 text-lg font-semibold text-gray-900">現在のしゅんダイアリー一覧</h2>
          {items.length === 0 ? (
            <EmptyState
              title="しゅんダイアリーがありません"
              description="右のフォームからしゅんダイアリーを追加してください"
            />
          ) : (
            <DndContext
              sensors={sensors}
                    ...
                    <React.Fragment key={item.id}>
                      ...
                      {editingItem?.id === item.id && (
                        <li className="rounded-lg border border-blue-200 bg-blue-50 p-4">
                          <p className="mb-3 text-sm font-medium text-blue-800">しゅんダイアリーを編集</p>
                          <ShunDiaryForm
                            initialValues={{
        ...
        <section className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          <h2 className="mb-4 text-lg font-semibold text-gray-900">新規しゅんダイアリーを追加</h2>
          <ShunDiaryForm onSubmit={handleCreate} submitLabel="追加する" />
        </section>
      </div>
    </div>
  );
