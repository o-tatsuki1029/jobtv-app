...
export default function SessionEditPage() {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    if (selectedSession) {
      ...
      if (name === "title") {
        error = validateRequired(value, "タイトル") || validateMaxLength(value, TITLE_MAX_LENGTH, "タイトル");
        ...
        error = validateRequired(value, "場所詳細") || validateMaxLength(value, TITLE_MAX_LENGTH, "場所詳細");
        if (error) newErrors.location_detail = error;
      } else if (name === "description") {
        error = validateRequired(value, "説明文") || validateMaxLength(value, LONG_DESCRIPTION_MAX_LENGTH, "説明文");
  ...
  const hasCharacterLimitErrors = (): boolean => {
    ...
    if (selectedSession.title && validateMaxLength(selectedSession.title, TITLE_MAX_LENGTH)) {
      return true;
    }
    if (selectedSession.description && validateMaxLength(selectedSession.description, LONG_DESCRIPTION_MAX_LENGTH)) {
      return true;
    }
  ...
  const validateRequiredFields = (): boolean => {
    ...
    const locationDetailError =
      ...
      validateMaxLength(selectedSession.location_detail || "", TITLE_MAX_LENGTH, "場所詳細");
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            <div className="p-6 border-b border-gray-100">
              ...
              <p className="text-sm text-gray-500 mt-1">
                説明会詳細ページの上部に表示される画像を設定できます。設定しなかった場合は、企業のカバー画像が自動で使用されます。
              </p>
            </div>
