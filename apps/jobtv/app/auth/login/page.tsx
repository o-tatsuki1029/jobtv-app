    ...
    <div className="flex-1 flex items-center justify-center min-h-0 px-4 py-6 bg-white">
      <div className="max-w-md w-full">
        ...
        <div className="bg-white p-6 md:p-8 rounded-xl border border-gray-200 max-sm:border-0 max-sm:rounded-none max-sm:px-4">
          <form method="POST" action="/api/auth/login" className="space-y-6" autoComplete="on" onSubmit={() => setLoading(true)}>
            ...
            <Link
              href={signupHref}
              className="block w-full py-4 text-base text-center font-semibold text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors"
            >
              新規登録（無料）
            </Link>
          </form>
