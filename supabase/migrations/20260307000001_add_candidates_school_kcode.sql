-- candidates に school_kcode を追加（管理者が school_master.group_name でグループ別分析可能）
ALTER TABLE candidates ADD COLUMN school_kcode TEXT;

-- RPC を更新して school_kcode を挿入できるようにする
...
COMMENT ON FUNCTION public.create_candidate_and_link_profile(jsonb) IS '認証済み candidate が自分用の candidates 行を 1 件作成し、profiles.candidate_id に紐付ける。email は profiles にのみ保存される。会員登録直後に 1 回だけ呼ぶ。';
