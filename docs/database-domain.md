...
- **DB に変更を加えた場合（マイグレーション・テーブル役割・使用ルールの変更など）は、必ずこの doc もあわせて修正すること。**
...
3. 同一リクエスト内で、認証済みセッションを使って RPC `create_candidate_and_link_profile(payload)` を呼ぶ。RPC 内で `candidates` に 1 行 INSERT し、`profiles.candidate_id` を UPDATE（1 トランザクション）。
