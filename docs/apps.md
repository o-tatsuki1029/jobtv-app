...
- **LINE 連携・配信**: 学生の LINE アカウント連携（LINE Login）と、Admin からのセグメント配信（Messaging API）。詳細は [line-integration.md](./line-integration.md) を参照。
- **ページテーマ**: ライト/ダークの規約と定義場所は [ui-and-styling.md](./ui-and-styling.md) を参照。
...
認証・ロール・ルート保護の仕様と参照実装は [roles-and-auth.md](./roles-and-auth.md) を参照。

### デプロイ

- **デプロイ先**: Vercel
- **管理者画面（/admin）の IP 制限**: インフラ側（Vercel Firewall 等）で実施する。詳細は [デプロイ・運用](./deployment.md) を参照。
