...
| #   | 内容                                       | 影響                                                      |
| --- | ---------------------------------------- | ------------------------------------------------------- |
| 1   | **求人エントリー / 説明会予約 → 学生への確認メールがない**       | 採用担当には通知されるが、学生側に「受け付けました」メールが送られない                     |
| 2   | **求人エントリー / 説明会予約 → Slack・Sheets 連携がない** | イベント予約にはあるが、求人エントリーにはない。データの一元管理が不完全                    |
| 3   | **イベント予約 → 採用担当への通知がない**                 | 学生にはメール+LINE、運営には Slack+Sheets があるが、該当企業の採用担当へのメール通知がない |
| 4   | ~~**管理者 LINE 配信の送信ログがない**~~（解決済み）         | `line_broadcast_logs` + `line_broadcast_deliveries` で配信ログ・候補者別結果を記録。管理画面 `/admin/line/history` で確認可能 |
| 5   | **メール送信成功時の Slack 通知がない**                | 失敗時のみ通知。送信量の集計・モニタリング手段がない                              |
